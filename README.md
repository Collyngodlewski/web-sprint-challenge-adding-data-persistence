...
Add `"start"`. `"server"`, `"migrate"` and `"rollback"` scripts to the `package.json` file. x
...
1. Explain the difference between Relational Databases and SQL.
  Relational databases holds data with relational tables. SQL is used to go through the tables and is able to manipulate and look at the data.

2. Why do tables need a Primary Key?
  Primary keys are unique and can be used to become foreign keys and link tables together

3. What is the name given to a table column that references the Primary Key on another table? The name given is foreign key and it links to the primary key

4. What do we need in order to have a _many to many_ relationship between two tables? We need to link the two together using an intermediate table. A primary key/ foreign key would be used. We could use product and a customer. Customers can purchase various products, and products can be purchased by many customers.
